...
class SoftwareUserDocController extends Controller
{
    private $mongo;
    private $db;
    private $jwtUtils;
    ...
    // upload file (Create)
    public function UploadSoftwareUserDoc(Request $request)
    {
        try {
            //! JWT
            ...
            // // Need to pass Software Component
            // $pipline = [
            ...
            //     ]]
            // ];
            // $checkStatus = $this->db->selectCollection("SoftwareComponent")->aggregate($pipline);
            ...
            //     return response()->json([
            //         'status' => 'error',
            //         'message' => 'Software Component is not verified',
            //         "data" => [],
            //     ], 400);
            // }

            // if documrnt has been created, cannot create again
            $checkDoc = $this->db->selectCollection("SoftwareUserDocs")->findOne(['project_id' => $this->MongoDBObjectId($projectID)]);
            if ($checkDoc !== null) {
                return response()->json([
                    'status' => 'error',
            ...
            $insertnew = $this->db->selectCollection("SoftwareUserDocs")->insertOne($option);
    ...
    public function GetSoftwareUserDoc(Request $request)
    {
        try {
            //! JWT
            $header = $request->header('Authorization');
            ...
            $userDoc = $this->db->selectCollection("SoftwareUserDocs")->aggregate($pipline);
            $dataUserDoc = array();
            foreach ($userDoc as $doc) {
                $pipline = [
                    ...
                    [
                        '$project' => [
                            'version' => 1, 'user_doc_id' => ['$toString' => '$_id'],
                ...
                $allVersion = $this->db->selectCollection("SoftwareUserDocs")->aggregate($pipline);
                $versionsAll = array();
                foreach ($allVersion as $ver) {
                    ...
                    array_push($versionsAll, [
                        "version" => $version, "user_doc_id" => $userDocID,
    ...
    public function GetIndividualDoc(Request $request)
    {
        try {
            ...
            $validator = Validator::make($request->all(), [
                'user_doc_id'       => 'required | string | min:1 | max:255',
            ]);
            if ($validator->fails()) {
                return response()->json([
                    'status' => 'error',
            ...
            $userDocID = $request->user_doc_id;
            $pipline = [
                ['$match' => ['_id' => $this->MongoDBObjectId($userDocID)]],
                ['$lookup' => ['from' => 'StatementOfWork', 'localField' => 'project_id', 'foreignField' => 'project_id', 'as' => 'StatementOfWork', 'pipeline' => [['$sort' => ['created_at' => -1]], ['$limit' => 1]]]],
                ['$replaceRoot' => ['newRoot' => ['$mergeObjects' => [['$arrayElemAt' => ['$StatementOfWork', 0]], '$$ROOT']]]],
                ['$lookup' => ['from' => 'Accounts', 'localField' => 'creator_id', 'foreignField' => 'user_id', 'as' => 'Accounts']],
                ['$replaceRoot' => ['newRoot' => ['$mergeObjects' => [['$arrayElemAt' => ['$Accounts', 0]], '$$ROOT']]]],
            ...
            $userCov = $this->db->selectCollection("SoftwareUserDocs")->aggregate($cover);
            $dataCover = array();
    ...
    // Edit document
    public function EditSoftwareUserDoc(Request $request)
    {
        try {
            //! JWT
            $header = $request->header('Authorization');
            ...
            $checkEdit = $this->db->selectCollection("SoftwareUserDocs")->aggregate($pipline);
            $checkEditData = array();
            ...
            if ($checkEditData[0]->is_edit !== false && $checkEditData[0]->status === null) {
                ...
                $updateDocument = $this->db->selectCollection("SoftwareUserDocs")->updateOne(
                    ['_id' => $this->MongoDBObjectId($userDocID)],
                    ['$set' => [
                        "documentation" => $softwareUserDoc,
                        "size_doc" => $sizeDoc,
                        "updated_at" => $timestamp,
            ...
            if ($checkEditData[0]->is_edit !== false && $checkEditData[0]->status !== null) {
                ...
                $setEditFalse = $this->db->selectCollection("SoftwareUserDocs")->updateOne(
                    ['_id' => $this->MongoDBObjectId($userDocID)],
                    ['$set' => [
                        "is_edit" => false,
                    ]]
                );
                $insertForEditApproved = $this->db->selectCollection("SoftwareUserDocs")->insertOne($option);
            }
