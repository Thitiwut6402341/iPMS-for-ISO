...
class MaintenanceDocController extends Controller
{
    ...
    // upload file (Create)
    public function UploadMaintenaceDoc(Request $request)
    {
        try {
            //! JWT
            ...
            $checkDoc = $this->db->selectCollection("MaintenanceDocs")->findOne(['project_id' => $this->MongoDBObjectId($projectID)]);
            if ($checkDoc !== null) {
                return response()->json([
                    'status' => 'error',
            ...
            $path = getcwd() . "\\..\\images\\MaintenanceDocumentation\\";
            if (!is_dir($path)) mkdir($path, 0777, true);
            // $pathUsed = 'http://10.1.9.77/Project/iPMS-ISO/images/MaintenanceDocumentation/'; // local
            $pathUsed = "https://snc-services.sncformer.com/dev/iPMSISO/images/MaintenanceDocumentation/"; //server
            ...
            $insertnew = $this->db->selectCollection("MaintenanceDocs")->insertOne($option);
    ...
    public function GetMaintenanceDoc(Request $request)
    {
        try {
            //! JWT
            $header = $request->header('Authorization');
            ...
            $userDoc = $this->db->selectCollection("MaintenanceDocs")->aggregate($pipline);
            $dataUserDoc = array();
            foreach ($userDoc as $doc) {
                $pipline = [
                    ...
                    [
                        '$project' => [
                            'version' => 1, 'maintenance_doc_id' => ['$toString' => '$_id'],
                            'project_name' => 1, 'status' => 1, 'is_edit' => 1, 'project_type' => 1,
                            'start_date' => 1, 'end_date' => 1
                        ]
                    ]
                ];
                $allVersion = $this->db->selectCollection("MaintenanceDocs")->aggregate($pipline);
                $versionsAll = array();
                foreach ($allVersion as $ver) {
                    ...
                    $version = $ver->version;
                    $maintenanceDocID = $ver->maintenance_doc_id;
                    ...
                    array_push($versionsAll, [
                        "version" => $version, "maintenance_doc_id" => $maintenanceDocID,
    ...
    public function GetIndividualDoc(Request $request)
    {
        try {
            ...
            $validator = Validator::make($request->all(), [
                'maintenance_doc_id'       => 'required | string | min:1 | max:255',
            ]);
            if ($validator->fails()) {
                return response()->json([
                    'status' => 'error',
            ...
            $maintenanceDocID = $request->maintenance_doc_id;
            $pipline = [
                ['$match' => ['_id' => $this->MongoDBObjectId($maintenanceDocID)]],
                ['$lookup' => ['from' => 'StatementOfWork', 'localField' => 'project_id', 'foreignField' => 'project_id', 'as' => 'StatementOfWork', 'pipeline' => [['$sort' => ['created_at' => -1]], ['$limit' => 1]]]],
                ['$replaceRoot' => ['newRoot' => ['$mergeObjects' => [['$arrayElemAt' => ['$StatementOfWork', 0]], '$$ROOT']]]],
                ['$lookup' => ['from' => 'Accounts', 'localField' => 'creator_id', 'foreignField' => 'user_id', 'as' => 'Accounts']],
                ['$replaceRoot' => ['newRoot' => ['$mergeObjects' => [['$arrayElemAt' => ['$Accounts', 0]], '$$ROOT']]]],
                ['$project' => [
                    "_id" => 0,
                    "maintenance_doc_id" => ['$toString' => '$_id'],
                    "project_id" => ['$toString' => '$project_id'],
                    "project_name" => 1,
            ...
            $userCov = $this->db->selectCollection("MaintenanceDocs")->aggregate($cover);
            $dataCover = array();
    ...
    // Edit document
    public function EditMaintenanceDoc(Request $request)
    {
        try {
            //! JWT
            $header = $request->header('Authorization');
            ...
            $checkEdit = $this->db->selectCollection("MaintenanceDocs")->aggregate($pipline);
            $checkEditData = array();
            ...
            if ($checkEditData[0]->is_edit !== false && $checkEditData[0]->status === null) {
                ...
                $updateDocument = $this->db->selectCollection("MaintenanceDocs")->updateOne(
                    ['_id' => $this->MongoDBObjectId($maintenanceDocID)],
                    ['$set' => [
                        "documentation" => $softwareUserDoc,
                        "size_doc" => $sizeDoc,
                        "updated_at" => $timestamp,
            ...
            if ($checkEditData[0]->is_edit !== false && $checkEditData[0]->status !== null) {
                ...
                $setEditFalse = $this->db->selectCollection("MaintenanceDocs")->updateOne(
                    ['_id' => $this->MongoDBObjectId($maintenanceDocID)],
                    ['$set' => [
                        "is_edit" => false,
                    ]]
                );
                $insertForEditApproved = $this->db->selectCollection("MaintenanceDocs")->insertOne($option);
            }
