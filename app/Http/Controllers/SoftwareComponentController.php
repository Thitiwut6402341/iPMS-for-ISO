...
class SoftwareComponentController extends Controller
{
    ...
    public function create(Request $request)
    {
        try {
            ...
            $rules = [
                ...
                'software_component'   => 'required|string|min:1|max:255',
            ...
            $results = $this->db->selectCollection("SoftwareComponent")->insertOne([
            ...
            return response()->json([
                "status" => "success",
                "message" => "Insert Software Component successfully !!",
                "data" => $results,
            ], 200);
        } catch (\Exception $e) {
            return response()->json([
    ...
    //! [POST]/software Component/update
    public function update(Request $request)
    {
        try {
            ...
            $rules = [
                'software_component_id' => 'required|string|min:1|max:255',
                'software_component'    => 'required|string|min:1|max:255',
                'version'               => 'required|string|min:1|max:255',
            ];
            ...
            $filter = ["_id" => $this->MongoDBObjectId($request->software_component_id)];
            $options = ["projection" => ["_id" => 0, "software_component_id" => ['$toString' => '$_id'], "project_id" => ['$toString' => '$project_id'], "project_name" => 1]];
            $softwareDocument = $this->db->selectCollection("SoftwareComponent")->findOne($filter, $options);

            if (is_null($softwareDocument)) {
                return response()->json([
                    "status" => "error",
                    "message" => "Software Component ID not found",
                    "data" => []
                ], 400);
            }
            ...
            $results = $this->db->selectCollection("SoftwareComponent")->updateOne(
                ["_id" => $this->MongoDBObjectId($request->software_component_id)],
                [
                    '$set' => [
                        "software_component" => $request->software_component,
                        "version"            => $request->version,
            ...
            return response()->json([
                "status" => "success",
                "message" => "Update software_component successfully !!",
                "data" => $results,
            ], 200);
        } catch (\Exception $e) {
            return response()->json([
